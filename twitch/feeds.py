...
# MARK: /rss/sitewide/rewards/
class SitewideRewardCampaignFeed(TTVDropsBaseFeed):
    """RSS feed for site-wide reward campaigns (no specific game)."""

    title: str = "TTVDrops: Site-wide Rewards"
    link: str = "/twitch/sitewide/"
    ...
    def items(self) -> list[RewardCampaign]:
        """Return site-wide reward campaigns (default 200, or limited by ?limit query param)."""
        limit: int = self._limit if self._limit is not None else 200
        queryset: QuerySet[RewardCampaign] = _active_reward_campaigns(
            RewardCampaign.objects
    ...
    def item_categories(self, item: RewardCampaign) -> tuple[str, ...]:
        ...
        categories: list[str] = ["twitch", "rewards", "quests", "sitewide"]
        if item.brand:
            categories.append(item.brand)
        return tuple(categories)
    ...
    def feed_url(self) -> str:
        """Return the URL to the RSS feed itself."""
        return reverse("core:sitewide_reward_feed")


class SitewideRewardCampaignAtomFeed(TTVDropsAtomBaseFeed, SitewideRewardCampaignFeed):
    """Atom feed for site-wide reward campaigns."""

    def feed_url(self) -> str:
        """Return the URL to the Atom feed itself."""
        return reverse("core:sitewide_reward_feed_atom")
